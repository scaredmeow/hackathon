...
# @router.get("/pets/{pet_name}/backpack", response_model=List[ItemInInventory])
# def get_pet_backpack(pet_name: str):
#     for pet in db["pets"]:
#         if pet["pet_name"] == pet_name:
#             return pet["backpack"]
#     raise HTTPException(status_code=404, detail="Pet not found")

# @router.put("/pets/{pet_name}/backpack/{item_name}", response_model=ItemInInventory)
# def update_backpack_item_quantity(pet_name: str, item_name: str, quantity: int):
#     for pet in db["pets"]:
#         if pet["pet_name"] == pet_name:
...
# @router.get("/pets/{pet_name}/fridge", response_model=List[ItemInInventory])
# def get_pet_fridge(pet_name: str):
#     for pet in db["pets"]:
#         if pet["pet_name"] == pet_name:
#             return pet["fridge"]
#     raise HTTPException(status_code=404, detail="Pet not found")

# @router.put("/pets/{pet_name}/fridge/{item_name}", response_model=ItemInInventory)
# def update_fridge_item_quantity(pet_name: str, item_name: str, quantity: int):
#     for pet in db["pets"]:
#         if pet["pet_name"] == pet_name:
